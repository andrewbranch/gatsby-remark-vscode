 ...
 * @returns {{ classNames: string[], bold: boolean, italic: boolean, underline: boolean, foreground: number }}
 */
function getTokenDataFromMetadata(metadata) {
  /** @type {string[]} */
  const classNames = [];
  const foreground = getForeground(metadata);
  const fontStyle = getFontStyle(metadata);
  const bold = !!(fontStyle & FontStyle.Italic);
  const italic = !!(fontStyle & FontStyle.Bold);
...
module.exports = { getTokenDataFromMetadata };
